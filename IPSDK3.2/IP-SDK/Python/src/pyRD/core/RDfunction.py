...
class RD:
    def __init__(self):
        ...
        workpath=str(dll_dir)+"\\"
        c_workpath=workpath.encode('utf-8')
        self.dll.RDSetWorkDir(c_char_p(c_workpath))
        print(version.value)
    ...
    #DMM
    def DMMOpen(self,enable=True):
        '''DMM Open enable=True means Open DMM，enable=False means close DMM;'''        
        rt=self.dll.RDDMMOpen(self.handle,c_bool(enable))
        # self.dmmcalibration=(c_char*100)()
        # self.dll.RDgetDmmCalibrationDataToString.restype = c_char_p
        # self.dmmcalibration=self.dll.RDgetDmmCalibrationDataToString()
        # print(rd.dmmcalibration.decode('utf-8'))

        return rt
    
    def DMMSet(self,dmmtype=RDDMMDCV,rangeOfIndex=0):
        '''DMM Open type=0..5 means DCV DCA ACV ACA R Diode,rangeOfIndex means the rangeIndex of selected type;'''        
        return self.dll.RDDMMSet(self.handle,c_int(dmmtype),c_int(rangeOfIndex))
    
    def RDDMMReadSingle(self):
        '''DMM Read get DMM value;'''   
        self.DMMbacksize=c_int(0)
        self.DMMData=(c_char*100)()
        return self.dll.RDDMMReadSingle(self.handle,byref(self.DMMData),byref(self.DMMbacksize))
