...
for i in range(10):
    # 设置信号源的频率
    freq = (i + 1) * 1000;
    rd.AnalogOutNodeFrequencySet(ao0.ch, ao0.node, freq)
    ...
    signals.append(np.array(rd.aidatach1))
...
nfft = 4096 * 8
half = nfft // 2
freq = np.fft.fftfreq(nfft, 1 / ai0.freq)
freq_khz = freq[:half] / 1000

fig, ax = plt.subplots(figsize=(12, 6))

for i, y in enumerate(signals):
    signal_fft = np.fft.fft(y, n=nfft)
    amp = 2.0 / ai0.buffersize * np.abs(signal_fft[:half])
    ax.plot(freq_khz, amp, label=f"{(i+1)*1000} Hz")

ax.set_xlim(0, 15)
ax.grid(True)
ax.set_xlabel("Frequency (kHz)")
ax.set_ylabel("Amplitude (V)")
ax.set_title(f"{device_sn} FFT Spectrum (Sweep 1kHz~10kHz)")
ax.legend()
plt.tight_layout()
plt.show()
